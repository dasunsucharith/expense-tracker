...
function setupEventListeners() {
    ...
    document.getElementById('update-expense').addEventListener('click', updateExpense);
    ...
    document.getElementById('delete-budget').addEventListener('click', deleteBudget);
    
    // Filters
    document.getElementById('expense-date-filter').addEventListener('change', toggleCustomDateRange);
    document.getElementById('apply-expense-filters').addEventListener('click', loadExpenses);
    
    // Reports
    document.getElementById('report-period').addEventListener('change', toggleReportCustomDateRange);
    document.getElementById('generate-report').addEventListener('click', generateReport);
    
    // Budget period selection
    document.getElementById('budget-period').addEventListener('change', toggleBudgetCustomDates);
}
...
function loadDashboardSummary() {
    ...
    return fetch(`${API_URL}/api/expense/dashboard/summary?start_date=${startDate}&end_date=${endDate}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    ...
    .then(data => {
        ...
        document.getElementById('expense-period').textContent = `${formatMonthYear(firstDay)}`;
    ...
    .then(data => {
        ...
        data.forEach(expense => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <span class="category-badge" style="background-color: ${expense.category.color}"></span>
...
function loadBudgetStatus() {
    ...
    .then(data => {
        ...
        data.forEach(budget => {
            ...
            const progressClass = percentUsed > 90 ? 'bg-danger' : 
...
function loadBudgets() {
    ...
    .then(data => {
        ...
        if (budgets.length === 0) {
            const row = document.createElement('tr');
            row.innerHTML = '<td colspan="8" class="text-center">No budgets found</td>';
            tableBody.appendChild(row);
            return;
        }
        ...
        budgets.forEach(budget => {
            ...
            const percentUsed = budget.percentage_used;
            const statusClass = percentUsed > 90 ? 'danger' : 
                               percentUsed > 75 ? 'warning' : 'success';
            
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${budget.name}</td>
                <td>${budget.category_id ? getCategoryName(budget.category_id) : 'All Categories'}</td>
                ...
                <td>
                    <span class="badge bg-${statusClass}">${Math.round(percentUsed)}% used</span>
                    ${isActive ? '<span class="badge bg-primary ms-1">Active</span>' : ''}
                </td>
                <td>
                    <button class="btn btn-sm btn-outline-primary edit-budget-btn" data-id="${budget.id}">
                        <i class="fas fa-edit"></i>
                    </button>
                </td>
            `;
...
function initializeCharts() {
    ...
    fetch(`${API_URL}/api/expense/dashboard/summary?start_date=${startDate}&end_date=${endDate}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    ...
    const budgetVsActualCtx = document.getElementById('budget-vs-actual-chart').getContext('2d');
...
function saveExpense() {
    ...
    .then(data => {
        if (data.id) {
            ...
            // Reload data
            if (document.querySelector('.nav-link[data-page="expenses"]').classList.contains('active')) {
                loadExpenses();
            } else {
                loadDashboardData();
            }
...
function openEditExpenseModal(expenseId) {
    ...
    document.getElementById('edit-expense-date').value = expense.date.substring(0, 10); // YYYY-MM-DD format
...
function saveBudget() {
    ...
    .then(data => {
        if (data.id) {
            ...
            // Reload data
            if (document.querySelector('.nav-link[data-page="budgets"]').classList.contains('active')) {
                loadBudgets();
            } else {
                loadDashboardData();
            }
...
function openEditBudgetModal(budgetId) {
    ...
    document.getElementById('edit-budget-start-date').value = budget.start_date.substring(0, 10); // YYYY-MM-DD format
    document.getElementById('edit-budget-end-date').value = budget.end_date.substring(0, 10); // YYYY-MM-DD format
...
function generateCategoryReport(startDate, endDate) {
    ...
    fetch(`${API_URL}/api/expense/dashboard/summary?start_date=${formattedStartDate}&end_date=${formattedEndDate}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    ...
    .then(data => {
        ...
        let summaryHTML = `
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <p class="mb-1">Total Expenses: <strong>${formatCurrency(data.total_expenses)}</strong></p>
                    <p class="mb-1">Number of Expenses: <strong>${data.expense_count}</strong></p>
                    <p class="mb-1">Period: <strong>${formatDate(startDate)} - ${formatDate(endDate)}</strong></p>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
        ...
        topCategories.forEach(category => {
            const percentage = Math.round((category.amount / data.total_expenses) * 100);
            summaryHTML += `
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <span>
                        <span class="category-badge" style="background-color: ${getCategoryColor(category.name)}"></span>
                        ${category.name}
                    </span>
                    <span>
...
function generateTimeReport(startDate, endDate) {
    ...
    fetch(`${API_URL}/api/expense/expenses?start_date=${formattedStartDate}&end_date=${formattedEndDate}`, {
        headers: {
            'Authorization': `Bearer ${token}`
        }
    })
    ...
    .then(data => {
        ...
        let summaryHTML = `
            <div class="card mb-3">
                <div class="card-body">
                    <h5 class="card-title">Summary</h5>
                    <p class="mb-1">Total Expenses: <strong>${formatCurrency(totalAmount)}</strong></p>
                    <p class="mb-1">Number of Expenses: <strong>${data.length}</strong></p>
                    <p class="mb-1">Period: <strong>${formatDate(startDate)} - ${formatDate(endDate)}</strong></p>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span>${daysWithExpenses} of ${dateRange.length}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span>Days without Expenses</span>
                            <span>${daysWithoutExpenses} of ${dateRange.length}</span>
                        </li>
                    </ul>
                </div>
            </div>
...
function generateBudgetReport(startDate, endDate) {
    ...
    Promise.all([
        fetch(`${API_URL}/api/budget/budgets?start_date=${formattedStartDate}&end_date=${formattedEndDate}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        }).then(response => response.json()),
    ...
    .then(([budgetsData, expensesData]) => {
        ...
        const overBudgetCount = budgetsData.filter(budget => budget.spent > budget.amount).length;
        ...
        let summaryHTML = `
            <div class="card mb-3">
                <div class="card-body">
                    <h5 class="card-title">Summary</h5>
                    <p class="mb-1">Total Budget: <strong>${formatCurrency(totalBudget)}</strong></p>
                    <p class="mb-1">Total Spent: <strong>${formatCurrency(totalSpent)}</strong></p>
                    <p class="mb-1">Total Remaining: <strong>${formatCurrency(totalRemaining)}</strong></p>
                    <p class="mb-1">Period: <strong>${formatDate(startDate)} - ${formatDate(endDate)}</strong></p>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span>${overBudgetCount} of ${budgetsData.length}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
...
function getRandomColor(index) {
    const colors = [
        ...
        '#33FFA8', '#FF3333', '#3357FF', '#FFAA33', '#808080'
    ];
